...
def _maybe_fetch_forks(args: argparse.Namespace, repo_dir: Path) -> Tuple[int, int, int]:
    if not getattr(args, "include_forks", False):
        return (0, 0, 0)
    token = (getattr(args, "github_token", "") or os.environ.get("GITHUB_TOKEN", "")).strip()
...
def _search_exact_blob_matches(repo_dir: Path, blob_hash_norm: str, blob_hash_raw: str) -> List[str]:
    exact_norm = find_exact_blob_commits(repo_dir, blob_hash_norm)
    exact_raw = [] if blob_hash_raw == blob_hash_norm else find_exact_blob_commits(repo_dir, blob_hash_raw)
...
def _scan_commits_for_candidates(
    ...
    with StepDisplay("search_exact", "Searching exact blob matches:", timings, 
                     AutoProgressBar(getattr(args, "progress", False)), getattr(args, "timings", False)) as disp:
    ...
    with StepDisplay("build_candidate_paths", "Building candidate paths:", timings, 
    ...
    if not candidate_paths:
        print("No exact blob match found and no candidate paths discovered.", file=sys.stderr)
        cleanup_repo_cache(repo_dir, args.repo_url)
        sys.exit(1)

    print("No exact blob match found. \nScanning commits touching candidate paths...", file=sys.stderr)

    # Collect all commits touching any candidate path (timed)
    touched_all: List[str] = []
    seen_paths = set()
    with StepDisplay("collect_touches", "Collecting commits touching candidate paths:", timings, 
                     AutoProgressBar(getattr(args, "progress", False)), getattr(args, "timings", False)) as disp:
    ...
    with StepDisplay("prepare_fingerprints", "Preparing local fingerprints:", timings, 
                     AutoProgressBar(getattr(args, "progress", False)), getattr(args, "timings", False)):
        ...
        local_mh = minhash_signature(local_fp, num_perm=max(1, int(getattr(args, "minhash_perm", 128) or 128))) if mode_choice == "minhash" else None
    ...
    with StepDisplay("evaluate_candidates", "Evaluating candidates:", timings, 
                     AutoProgressBar(getattr(args, "progress", False)), getattr(args, "timings", False)) as disp:
        ...
        for idx, c in enumerate(touched, 1):
            ...
            if mode_choice == "jaccard":
                ...
                repo_fp = fingerprint_text_for_fuzzy(repo_text, k=shingle_k)
                repo_mh = minhash_signature(repo_fp, num_perm=max(1, int(getattr(args, "minhash_perm", 128) or 128)))
    ...
    if not candidates and best_fuzzy:
        with StepDisplay("apply_fuzzy_threshold", "Applying fuzzy matching threshold:", timings, 
                         AutoProgressBar(getattr(args, "progress", False)), getattr(args, "timings", False)):
...
def orchestrate(args: argparse.Namespace) -> None:
    ...
    with StepDisplay("read_local_file", "Reading local file:", timings, 
                     AutoProgressBar(getattr(args, "progress", False)), getattr(args, "timings", False)):
    ...
    if getattr(args, "include_forks", False):
        with StepDisplay("fetch_forks", "Fetching forks:", timings, 
                         AutoProgressBar(getattr(args, "progress", False)), getattr(args, "timings", False)):
            forks_fetched, forks_discovered, forks_selected = _maybe_fetch_forks(args, repo_dir)
    else:
        forks_fetched, forks_discovered, forks_selected = (0, 0, 0)

    with StepDisplay("compute_local_blobs", "Computing local blob ids:", timings, 
                     AutoProgressBar(getattr(args, "progress", False)), getattr(args, "timings", False)):
    ...
    with StepDisplay("process_candidates", "Processing candidates:", timings, 
                     AutoProgressBar(getattr(args, "progress", False)), getattr(args, "timings", False)):
    ...
    with StepDisplay("choose_preferred", "Choosing preferred:", timings, 
                     AutoProgressBar(getattr(args, "progress", False)), getattr(args, "timings", False)):
        preferred = choose_preferred(repo_dir, candidates)

    with StepDisplay("write_report", "Writing report:", timings, 
                     AutoProgressBar(getattr(args, "progress", False)), getattr(args, "timings", False)):
        _write_report(
        args=args,
        mode=mode,
    ...
    with StepDisplay("write_env", "Writing env:", timings, 
                     AutoProgressBar(getattr(args, "progress", False)), getattr(args, "timings", False)):
        _write_env_file(
        args=args,
        mode=mode,
    ...
    with StepDisplay("cleanup", "Cleanup cache:", timings, 
                     AutoProgressBar(getattr(args, "progress", False)), getattr(args, "timings", False)):
        cleanup_repo_cache(repo_dir, args.repo_url)
    ...
    individual_sum = sum(ms for key, ms in timings.items() if key.endswith('_ms') and key != 'total_ms')
