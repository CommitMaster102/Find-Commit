...
def test_fast_mode_file_writing_optimizations():
    ...
    args = argparse.Namespace(
        fast=True,
        out_report="test_report.txt",
        out_env="test_env.env"
    )
    ...
    with patch('find_commits_lib.git_ops.commit_timestamp') as mock_timestamp, \
         patch('find_commits_lib.git_ops.branches_containing') as mock_branches, \
         patch('find_commits_lib.selection.choose_branch_for_commit') as mock_choose_branch:
        ...
        # Test report writing
        _write_report(
            args, mode, preferred, blob_hash_norm, blob_hash_raw,
            forks_fetched, forks_discovered, forks_selected, candidates, repo_dir, timings
        )
        
        # Test env file writing
        _write_env_file(
            args, mode, preferred, candidates, forks_fetched, 
            forks_discovered, forks_selected, repo_dir, timings
        )
